...
class BenchWorker(Worker):
    def load_model(self, **kwargs):
        ...
        self._graph_originals={}
        for name in ('run_fullgraph', 'run_pw_graph'):
            original_graph=getattr(CudaGraphManager,name)
            self._graph_originals[name]=original_graph
            def count_graph(manager,*args,_name=name,_original=original_graph,**kw):
                key=_name + (':' + str(args[0]) if _name=='run_fullgraph' else '')
                self._replays[key]=self._replays.get(key,0)+1
                return _original(manager,*args,**kw)
            setattr(CudaGraphManager,name,count_graph)
    ...
    def probe(self,action):
        ...
        if action=='timing':
            ...
            from vllm.v1.worker.gpu.cudagraph_utils import CudaGraphManager
            for name,original in self._graph_originals.items():
                setattr(CudaGraphManager,name,original)
...
def main(args):
    ...
    llm=LLM(model=model,tokenizer_mode='deepseek_v41',tensor_parallel_size=4,
        ...
        worker_cls='fused_output_bench_mrv2.BenchWorker')
    ...
    def save():path.write_text(json.dumps({'runner':'MRV2','variant':args.variant,'diagnostic':diagnostic,'mixed_outputs':mixed_outputs,'results':results},indent=2))
    save()
